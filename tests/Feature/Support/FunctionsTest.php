...
use function Xray\AzureStoragePhpSdk\Support\{
    convert_to_ISO,
    convert_to_RFC1123,
    convert_to_RFC3339_micro,
...
})->covers('\\Xray\\AzureStoragePhpSdk\\Support\\with');

it('should check if it\'s running in console', function () {
    expect(is_running_in_console())->toBeTrue();
})->covers('\\Xray\\AzureStoragePhpSdk\\Support\\is_running_in_console');
...
)->covers('\\Xray\\AzureStoragePhpSdk\\Support\\validate_protocol');
...
])->covers('\\Xray\\AzureStoragePhpSdk\\Support\\str_camel_to_header');
...
it('should convert value to boolean type', function (mixed $value, bool $expected) {
    ...
    'Array'        => [[1, 2, 3], false],
])->covers('\\Xray\\AzureStoragePhpSdk\\Support\\to_boolean');
...
})->covers('\\Xray\\AzureStoragePhpSdk\\Support\\convert_to_RFC1123');
...
})->covers('\\Xray\\AzureStoragePhpSdk\\Support\\convert_to_RFC3339_micro');
...
])->covers('\\Xray\\AzureStoragePhpSdk\\Support\\convert_to_ISO');
